...
/**
 * Pure string-building for the two PIE assessment emails. Mirrors
 * cleversiteEmails.ts - see that file and resultEmailLayout.ts for the
 * reasoning. No Resend calls here, no em dashes, prospect email never
 * touches internal-only fields.
 */
...
export function buildProspectEmail(result: AssessmentResult, name: string): EmailPayload {
  ...
  const args = { name, scoreLabel: "Process Health Score", assessmentName: "PIE", result };
  return {
    subject,
...
export function buildInternalEmail(
  ...
  const subject = `[${priorityTag}] PIE Assessment - ${name} (${company})`;
  ...
  const html = [
    ...
    `<p style="color: #999; font-size: 13px;">Sent from the Revenue Institute PIE assessment</p>`,
    `</div>`,
  ].join("\n");
  ...
  const text = [
    ...
    contextRowsText,
    "",
    "- Sent from the Revenue Institute PIE assessment",
  ].join("\n");
