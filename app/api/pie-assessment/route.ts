...
function validateAnswers(answers: unknown): answers is AnswerMap {
  ...
  for (const question of pieConfig.questions) {
...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    console.error("[pie-assessment] RESEND_API_KEY is not set");
  ...
  if (prospectOutcome.status === "rejected") {
    console.error("[pie-assessment] Prospect email failed:", prospectOutcome.reason);
  } else if (prospectOutcome.value.error) {
    console.error("[pie-assessment] Prospect email Resend error:", prospectOutcome.value.error);
  }

  if (internalOutcome.status === "rejected") {
    console.error("[pie-assessment] Internal email failed:", internalOutcome.reason);
  } else if (internalOutcome.value.error) {
    console.error("[pie-assessment] Internal email Resend error:", internalOutcome.value.error);
  }

  sendToN8n({
    source: "pie-assessment",
    form: "PIE Assessment",
    name,
    email,
    url,
